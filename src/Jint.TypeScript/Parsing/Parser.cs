...
internal sealed partial class Parser : IParser, StackGuard.IRecursionDepthProvider
{
    ...
    public Script ParseScript(string input, int start, int length, string? sourceFile = null, bool strict = false)
    {
        ...
        try
        {
            ...
            return FinishNodeAt(startMarker, new Marker(_tokenizer._start, _tokenizer._startLocation), new Script(body, _strict) { Range = NodeRange(startMarker, new Marker(_tokenizer._start, _tokenizer._startLocation)), Location = NodeLocation(startMarker, new Marker(_tokenizer._start, _tokenizer._startLocation)) }, Scope.GetScopeRef(_scopeStack, 0));
        }
        finally
        {
    ...
    public Module ParseModule(string input, int start, int length, string? sourceFile = null)
    {
        ...
        try
        {
            ...
            Debug.Assert(_tokenizer._type == TokenType.EOF);
            return FinishNodeAt(startMarker, new Marker(_tokenizer._start, _tokenizer._startLocation), new Module(body) { Range = NodeRange(startMarker, new Marker(_tokenizer._start, _tokenizer._startLocation)), Location = NodeLocation(startMarker, new Marker(_tokenizer._start, _tokenizer._startLocation)) }, Scope.GetScopeRef(_scopeStack, 0));
        }
        finally
        {
            ReleaseReferencesAndLargeBuffers();
