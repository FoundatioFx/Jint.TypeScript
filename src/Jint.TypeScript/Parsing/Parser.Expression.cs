...
internal partial class Parser
{
    ...
    internal Expression ParseExpression(ref DestructuringErrors destructuringErrors, ExpressionContext context = ExpressionContext.Default)
    {
        ...
        if (_tokenizer._type == TokenType.Comma)
        {
            ...
            return FinishNode(startMarker, new SequenceExpression(ParserNodeList.From(ref expressions)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
        }
    ...
    internal Expression ParseMaybeAssign(ref DestructuringErrors destructuringErrors, ExpressionContext context = ExpressionContext.Default)
    {
        ...
        if (IsContextual("yield"))
        {
            if (InGenerator)
            {
                ...
                // (see also https://github.com/acornjs/acorn/pull/1443#issuecomment-5170294599). However, JSX parsing relies on
        ...
        if (_tokenizer._type.IsAssignment)
        {
            ...
            return ExitRecursion(FinishNode(startMarker, new AssignmentExpression(op, leftNode, right) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }));
        }
    ...
    private Expression ParseMaybeConditional(ref DestructuringErrors destructuringErrors, ExpressionContext context)
    {
        ...
        if (_tokenizer._type == TokenType.Question && !(expr.Type == NodeType.ArrowFunctionExpression && expr.Range.Start == startMarker.Index))
        {
            ...
            return FinishNode(startMarker, new ConditionalExpression(test: expr, consequent, alternate) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
        }
    ...
    private BinaryExpression BuildBinary(in Marker startMarker, Expression left, Expression right, Operator op, bool logical)
    {
        ...
        return FinishNode<BinaryExpression>(startMarker, logical
            ? new LogicalExpression(op, left, right) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }
            : new NonLogicalBinaryExpression(op, left, right) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private Expression ParseMaybeUnary(bool sawUnary, bool incDec, ref DestructuringErrors destructuringErrors, ExpressionContext context)
    {
        ...
        else if (_tokenizer._type.Prefix)
        {
            ...
            expr = FinishNode<UnaryExpression>(startMarker, update
                ? new UpdateExpression(op, argument, prefix: true) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }
                : new NonUpdateUnaryExpression(op, argument) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
        }
        ...
        else
        {
            ...
            while (_tokenizer._type.Postfix && !CanInsertSemicolon())
            {
                ...
                Next();
                expr = FinishNode(startMarker, new UpdateExpression(op, expr, prefix: false) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
            }
        }

        if (_tokenizer._type == TokenType.StarStar && !incDec && !(expr.Type == NodeType.ArrowFunctionExpression && expr.Range.Start == startMarker.Index))
        {
            Next();
    ...
    private Expression ParseSubscripts(in Marker startMarker, Expression baseExpr, bool noCalls, ExpressionContext context)
    {
        ...
        for (; ; )
        {
            ...
            if (ReferenceEquals(element, baseExpr) || element.Type == NodeType.ArrowFunctionExpression)
            {
                ...
                {
                    element = FinishNode(startMarker, new ChainExpression(element) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
                }
        ...
        ref int optionalChainPos, ref bool hasCall, ExpressionContext context)
    {
            ...
            || Eat(TokenType.Dot))
        {
            ...
            baseExpr = FinishNode(startMarker, new MemberExpression(obj: baseExpr, property, computed, optional) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
        }
        else if (!noCalls && Eat(TokenType.ParenLeft))
        {
            ...
            baseExpr = FinishNode(startMarker, new CallExpression(callee: baseExpr, exprList, optional) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
            hasCall = true;
        }
        else if (_tokenizer._type == TokenType.BackQuote)
        {
            ...
            baseExpr = FinishNode(startMarker, new TaggedTemplateExpression(tag: baseExpr, quasi) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
        }
        ...
        switch (_tokenizer._type.Kind)
        {
            case TokenKind.Keyword:
                switch (_tokenizer._type.Keyword!.Value)
                {
                    case Keyword.Super:
                        ...
                        return FinishNode(startMarker, new Super() { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });

                    case Keyword.This:
                        Next();
                        return FinishNode(startMarker, new ThisExpression() { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
            ...
            case TokenKind.Identifier when _tokenizer._type == TokenType.Name:
                ...
                if (canBeArrow && !CanInsertSemicolon())
                {
                    ...
                    {
                        return ParseArrowExpression(startMarker, parameters: NodeList.From<Node>(id), isAsync: false, context & ~ExpressionContext.ForNew);
                    }
                        ...
                        && ((context & ExpressionContext.AwaitForInit) != ExpressionContext.AwaitForInit || !"of".Equals(_tokenizer._value.Value) || _tokenizer._containsEscape))
                    {
                        ...
                        return ParseArrowExpression(startMarker, parameters: NodeList.From<Node>(id), isAsync: true, context & ~ExpressionContext.ForNew);
                    }
                }
            ...
            case TokenKind.Punctuator when _tokenizer._type == TokenType.BracketLeft:
                ...
                Next();
                expr = FinishNode(startMarker, new ArrayExpression(ParseExprList(close: TokenType.BracketRight, allowTrailingComma: true, allowEmptyItem: true, ref destructuringErrors)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    ...
    private Expression ParseExprImport(ExpressionContext context)
    {
        ...
        {
            import = ParseDynamicImport(startMarker);
        }
        else if (_tokenizer._type == TokenType.Dot)
        {
            var meta = FinishNode(startMarker, new Identifier(TokenType.Import.Label) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
            var expr = ParseImportMetaOrPhase(startMarker, meta, context);
            import = expr as ImportExpression;
            if (import is null)
            {
                return expr;
            }
        ...
        {
            // Unexpected(); // original acornjs error reporting
            var importExpr = "import" + (import.Phase == ImportPhase.None ? string.Empty : ".") + ImportDeclaration.GetImportPhaseToken(import.Phase);
            Raise(import.Start, ImportCallNotNewExpression, new[] { importExpr });
        }

        return import;
    }

    private ImportExpression ParseDynamicImport(in Marker startMarker, ImportPhase phase = ImportPhase.None)
    {
        // https://github.com/acornjs/acorn/blob/e37a9c31423db95ee8de97a2e645a702240e2aa8/acorn/src/expression.js > `pp.parseDynamicImport = function`
        ...
        {
            // Unexpected(); // original acornjs error reporting
            var importExpr = "import" + (phase == ImportPhase.None ? string.Empty : ".") + ImportDeclaration.GetImportPhaseToken(phase);
            Raise(_tokenizer._lastTokenStart, ImportMissingSpecifier, new object[] { importExpr });
        }
        ...
        if (!Eat(TokenType.ParenRight))
        {
                ...
                || phase == ImportPhase.Defer)
            {
                ...
                if (!AfterTrailingComma(TokenType.ParenRight))
                {
            ...
            else
            {
                ...
                if (!Eat(TokenType.Comma)
                    || !AfterTrailingComma(TokenType.ParenRight, isAllowed: phase != ImportPhase.None))
                {
                    Unexpected();
                }
            }
        ...
        return FinishNode(startMarker, new ImportExpression(source, options, phase) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private Expression ParseImportMetaOrPhase(in Marker startMarker, Identifier meta, ExpressionContext context)
    {
        ...
        if (property.Name == "meta")
        {
            ...
            return FinishNode(startMarker, new MetaProperty(meta, property) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
        }

        if (_tokenizerOptions.AllowSourcePhaseImports() && property.Name == "source"
            || _tokenizerOptions.AllowDeferImportEvaluation() && property.Name == "defer")
        {
            if (containsEsc)
            {
                var importExpr = string.Concat("import", ".", property.Name);
                RaiseRecoverable(property.Start, InvalidEscapedMetaProperty, new object[] { importExpr });
            }
            if (_tokenizer._type != TokenType.ParenLeft)
            {
                Unexpected();
            }
            return ParseDynamicImport(startMarker, property.Name[0] == 's' ? ImportPhase.Source : ImportPhase.Defer);
        }
    ...
    private Expression ParseParenAndDistinguishExpression(bool canBeArrow, ExpressionContext context, Marker? genericStart = null)
    {
        ...
        if (_tokenizerOptions._ecmaVersion >= EcmaVersion.ES6)
        {
            ...
            if (parameters.Count > 1)
            {
                ...
                val = FinishNodeAt(innerStartMarker, innerEndMarker, new SequenceExpression(ParserNodeList.From(ref exprList)) { Range = NodeRange(innerStartMarker, innerEndMarker), Location = NodeLocation(innerStartMarker, innerEndMarker) });
            }
            else
            {
        ...
        return _options._preserveParens
            ? FinishNode(startMarker, new ParenthesizedExpression(val) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) })
            : val;
    }
    ...
    private Expression ParseNew()
    {
        ...
        {
            var meta = FinishNode(startMarker, new Identifier(TokenType.New.Label) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
            Next();
        ...
        return FinishNode(startMarker, new NewExpression(callee, arguments) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private TemplateElement ParseTemplateElement()
    {
        ...
        var value = Acornima.TemplateValue.From(cooked: _tokenizer._value.TemplateCooked, raw: (string)_tokenizer._value.Value!);

        Next();
        var tail = _tokenizer._type == TokenType.BackQuote;
        return FinishNode(startMarker, new TemplateElement(value, tail) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private TemplateLiteral ParseTemplate(bool isTagged)
    {
        ...
        Next();
        return FinishNode(startMarker, new TemplateLiteral(ParserNodeList.From(ref quasis), ParserNodeList.From(ref expressions)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private Node ParseObject(bool isPattern, ref DestructuringErrors destructuringErrors)
    {
        ...
        return FinishNode<Node>(startMarker, isPattern
            ? new ObjectPattern(ParserNodeList.From(ref properties)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }
            : new ObjectExpression(ParserNodeList.From(ref properties)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private Node ParseProperty(bool isPattern, int objectStartIndex, ref DestructuringErrors destructuringErrors)
    {
        ...
        if (_tokenizerOptions._ecmaVersion >= EcmaVersion.ES9 && Eat(TokenType.Ellipsis))
        {
            if (isPattern)
            {
                ...
                return FinishNode(propertyStartMarker, new RestElement(argument) { Range = NodeRange(propertyStartMarker), Location = NodeLocation(propertyStartMarker) });
            }
            else
            {
                // Parse argument.
                ...
                // Finish
                return FinishNode(propertyStartMarker, new SpreadElement(argument) { Range = NodeRange(propertyStartMarker), Location = NodeLocation(propertyStartMarker) });
            }
        }
        ...
        return FinishNode<Property>(propertyStartMarker, isPattern
            ? new AssignmentProperty(key, value, computed, shorthand) { Range = NodeRange(propertyStartMarker), Location = NodeLocation(propertyStartMarker) }
            : new ObjectProperty(kind, key, value, computed, shorthand, method) { Range = NodeRange(propertyStartMarker), Location = NodeLocation(propertyStartMarker) });
    }
        ...
        bool containsEsc, int objectStartIndex, in Marker startMarker, ref DestructuringErrors destructuringErrors)
    {
        ...
        else if (!computed && _tokenizerOptions._ecmaVersion >= EcmaVersion.ES6 && key is Identifier keyIdentifier)
        {
            ...
            else if (_tokenizer._type == TokenType.Eq)
            {
                ...
                value = FinishNode(startMarker, new AssignmentPattern(key, right) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
            }
            else
            {
                value = key;
    ...
    private FunctionExpression? ParseMethod(bool isGenerator, bool isAsync = false, ScopeFlags superFlags = ScopeFlags.Super, bool isSetter = false, bool allowThisParameter = true, bool allowOverload = false)
    {
        ...
        return FinishNode(startMarker, new FunctionExpression(id: null, parameters, (FunctionBody)body, isGenerator, isAsync) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }, scope);
    }
    ...
    private ArrowFunctionExpression ParseArrowExpression(in Marker startMarker, in NodeList<Node> parameters, bool isAsync, ExpressionContext context)
    {
        ...
        return FinishNode(startMarker, new ArrowFunctionExpression(paramList, body, expression, isAsync) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }, scope);
    }
        ...
        out bool expression, out StatementOrExpression body)
    {
        ...
        else
        {
            ...
            body = FinishNode(startMarker, new FunctionBody(ParserNodeList.From(ref statements), strict) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }, scope);
        }
...
#pragma warning restore format
    ...
    private Identifier ParseIdentifier(bool liberal = false)
    {
        ...
        var identifier = FinishNode(startMarker, new Identifier(name) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    ...
    private PrivateIdentifier ParsePrivateIdentifier()
    {
        ...
        Next();
        var privateIdentifier = FinishNode(startMarker, new PrivateIdentifier(name) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    ...
    private YieldExpression ParseYield(ExpressionContext context)
    {
        ...
        return FinishNode(startMarker, new YieldExpression(argument, @delegate) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private AwaitExpression ParseAwait(ExpressionContext context)
    {
        ...
        return FinishNode(startMarker, new AwaitExpression(argument) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private Decorator ParseDecorator()
    {
        ...
        return FinishNode(startMarker, new Decorator(expression) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
}
