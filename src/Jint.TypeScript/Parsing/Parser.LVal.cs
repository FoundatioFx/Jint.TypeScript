...
internal partial class Parser
{
        ...
        bool isInPattern = false, bool allowCall = false, LeftHandSideKind lhsKind = LeftHandSideKind.Unknown)
    {
        ...
        Reenter:
            if (isBinding) CheckAssertedBinding(node);
            switch (node.Type)
            {
                ...
                case NodeType.ObjectExpression:
                    ...
                    node = ReinterpretNode(node, new ObjectPattern(properties: convertedNodes!) { Range = node.Range, Location = node.Location });
                    break;
                ...
                case NodeType.Property:
                    ...
                    node = ReinterpretNode(node, new AssignmentProperty(property.Key, value: convertedNode, computed: property.Computed, shorthand: property.Shorthand) { Range = node.Range, Location = node.Location });
                    break;
                ...
                case NodeType.ArrayExpression:
                    ...
                    node = ReinterpretNode(node, new ArrayPattern(elements: convertedNodes) { Range = node.Range, Location = node.Location });
                    break;
                ...
                case NodeType.SpreadElement:
                    ...
                    node = ReinterpretNode(node, new RestElement(argument: convertedNode) { Range = node.Range, Location = node.Location });
                    break;
                ...
                case NodeType.AssignmentExpression:
                    ...
                    node = ReinterpretNode(node, new AssignmentPattern(left: convertedNode, assignmentExpression.Right) { Range = node.Range, Location = node.Location });
                    break;
    ...
    private NodeList<Node> ToAssignableProperties(in NodeList<Node> properties, bool isBinding)
    {
        ...
        return ParserNodeList.From(ref assignmentProperties);
    }
    ...
    private NodeList<Node?> ToAssignableList(in NodeList<Node?> exprList, bool isBinding)
    {
        ...
        return ParserNodeList.From(ref bindingList);
    }
    ...
    private SpreadElement ParseSpread(ref DestructuringErrors destructuringErrors)
    {
        ...
        return FinishNode(startMarker, new SpreadElement(argument) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private RestElement ParseRestBinding()
    {
        ...
        return FinishNode(startMarker, new RestElement(argument) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private Node ParseBindingAtom()
    {
        ...
        if (_tokenizerOptions._ecmaVersion >= EcmaVersion.ES6)
        {
            if (_tokenizer._type == TokenType.BracketLeft)
            {
                ...
                _bindingPatternDepth--;
                return ExitRecursion(FinishNode(startMarker, new ArrayPattern(elements) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }));
            }
    ...
    private NodeList<Node?> ParseBindingList(TokenType close, bool allowEmptyElement, bool allowTrailingComma, bool allowOptionalParameters = true, bool allowThisParameter = false, bool constructorParameters = false)
    {
        ...
        var first = true;
        if (allowThisParameter && _tokenizer._type == TokenType.This)
        {
            Next();
            if (!ParseTypeAnnotation()) TypeScriptError("ExpectedThisType", "A 'this' parameter requires a type annotation");
            first = false;
        }
        while (!Eat(close))
        {
            ...
            else if (_tokenizer._type == TokenType.Ellipsis)
            {
                ...
                if (close == TokenType.ParenRight) ParseParameterAnnotation(rest, allowOptional: false);
                elements.Add(rest);
    ...
    private Node ParseMaybeDefault(in Marker startMarker, Node? left = null)
    {
        ...
        return FinishNode(startMarker, new AssignmentPattern(left, right) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
