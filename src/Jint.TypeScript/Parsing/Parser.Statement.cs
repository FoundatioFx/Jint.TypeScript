...
internal partial class Parser
{
    ...
    Success:
        return FinishNode<Statement>(startMarker, isBreak ? new BreakStatement(label) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) } : new ContinueStatement(label) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private DebuggerStatement ParseDebuggerStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new DebuggerStatement() { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private DoWhileStatement ParseDoStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new DoWhileStatement(body, test) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private IfStatement ParseIfStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new IfStatement(test, consequent, alternate) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private ReturnStatement ParseReturnStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new ReturnStatement(argument) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private SwitchStatement ParseSwitchStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new SwitchStatement(discriminant, ParserNodeList.From(ref cases)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }, scope);
    }
    ...
    private ThrowStatement ParseThrowStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new ThrowStatement(argument) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private TryStatement ParseTryStatement(in Marker startMarker)
    {
        ...
        if (_tokenizer._type == TokenType.Catch)
        {
            ...
            handler = FinishNode(clauseStartMarker, new CatchClause(param, body) { Range = NodeRange(clauseStartMarker), Location = NodeLocation(clauseStartMarker) }, scope);
        }
        ...
        return FinishNode(startMarker, new TryStatement(block, handler, finalizer) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private WhileStatement ParseWhileStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new WhileStatement(test, body) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private WithStatement ParseWithStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new WithStatement(obj, body) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private EmptyStatement ParseEmptyStatement(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new EmptyStatement() { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private LabeledStatement ParseLabeledStatement(in Marker startMarker, string maybeName, Identifier expr, StatementContext context)
    {
        ...
        return FinishNode(startMarker, new LabeledStatement(expr, body) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private NonSpecialExpressionStatement ParseExpressionStatement(in Marker startMarker, Expression expression)
    {
        ...
        return FinishNode(startMarker, new NonSpecialExpressionStatement(expression) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private NestedBlockStatement ParseBlockStatement(in Marker startMarker, bool createNewLexicalScope = true)
    {
        ...
        return FinishNode(startMarker, new NestedBlockStatement(ParserNodeList.From(ref statements)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }, scope);
    }
    ...
    private ForStatement ParseFor(in Marker startMarker, StatementOrExpression? init)
    {
        ...
        return FinishNode(startMarker, new ForStatement(init, test, update, body) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }, scope);
    }
    ...
    private Statement ParseForInOf(in Marker startMarker, bool isForIn, bool await, Node init)
    {
        ...
        return FinishNode<Statement>(startMarker, isForIn
            ? new ForInStatement(left, right, body) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }
            : new ForOfStatement(left, right, body, await) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) },
            scope);
    }
    ...
    private VariableDeclarationParts ParseVar(VariableDeclarationKind kind, bool isFor)
    {
        ...
        do
        {
            ...
            declarations.Add(FinishNode(declStartMarker, new VariableDeclarator(id, init) { Range = NodeRange(declStartMarker), Location = NodeLocation(declStartMarker) }));
        }
    ...
    private StatementOrExpression ParseClass(in Marker startMarker, FunctionOrClassFlags flags, bool isAbstract = false)
    {
        ...
        var classBody = FinishNode(classBodyStartMarker, new ClassBody(ParserNodeList.From(ref body)) { Range = NodeRange(classBodyStartMarker), Location = NodeLocation(classBodyStartMarker) });
        ExitClassBody();
        ...
        return FinishNode<StatementOrExpression>(startMarker, isStatement
            ? new ClassDeclaration(id, superClass, classBody, ParserNodeList.From(ref _decorators)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }
            : new ClassExpression(id, superClass, classBody, ParserNodeList.From(ref _decorators)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) },
            scope);
    }
        ...
        bool computed, bool isStatic, bool isAsync, bool isGenerator, bool constructorAllowsSuper, ref ArrayList<Decorator> decorators, bool abstractSignature = false)
    {
        ...
        return FinishNode(startMarker, new MethodDefinition(kind, key, value, computed, isStatic, ParserNodeList.From(ref decorators)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private ClassProperty ParseClassField(in Marker startMarker, Expression key, bool computed, bool isStatic, bool isAccessor, ref ArrayList<Decorator> decorators)
    {
        ...
        return FinishNode<ClassProperty>(startMarker, isAccessor
            ? new AccessorProperty(key, value, computed, isStatic, ParserNodeList.From(ref decorators)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }
            : new PropertyDefinition(key, value, computed, isStatic, ParserNodeList.From(ref decorators)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private StaticBlock ParseClassStaticBlock(in Marker startMarker)
    {
        ...
        return FinishNode(startMarker, new StaticBlock(ParserNodeList.From(ref body)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }, scope);
    }
    ...
    private ExportAllDeclaration ParseExportAllDeclaration(in Marker startMarker, HashSet<string>? exports)
    {
        ...
        return FinishNode(startMarker, new ExportAllDeclaration(exported, source, ParserNodeList.From(ref attributes)) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private ExportSpecifier ParseExportSpecifier(HashSet<string>? exports)
    {
        ...
        return FinishNode(startMarker, new ExportSpecifier(local, exported) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private NodeList<ExportSpecifier> ParseExportSpecifiers(HashSet<string>? exports)
    {
        ...
        return ParserNodeList.From(ref nodes);
    }
    ...
    private ImportDeclaration ParseImportDeclaration(in Marker startMarker)
    {
            ...
            && !IsSourcePhaseAmbiguousAsDefaultImport())
        {
            ...
            specifiers = ParserNodeList.From(ref nodes);
    ...
    ParseSource:
        ...
        return FinishNode(startMarker, new ImportDeclaration(specifiers, source, ParserNodeList.From(ref attributes), phase) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private ImportSpecifier ParseImportSpecifier()
    {
        ...
        return FinishNode(startMarker, new ImportSpecifier(imported, local!) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private ImportDefaultSpecifier ParseImportDefaultSpecifier()
    {
        ...
        CheckLValSimple(local, BindingType.Lexical);
        return FinishNode(startMarker, new ImportDefaultSpecifier(local) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private ImportNamespaceSpecifier ParseImportNamespaceSpecifier()
    {
        ...
        var local = ParseIdentifier();
        CheckLValSimple(local, BindingType.Lexical);
        return FinishNode(startMarker, new ImportNamespaceSpecifier(local) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
    ...
    private ArrayList<Statement> ParseDirectivePrologue(bool allowStrictDirective, bool topLevel = false)
    {
        ...
        while (true)
        {
            ...
            FinishDirective:
                body.Add(FinishNode(startMarker, new Directive(expr, directive) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }));
            }
            else
            {
                Semicolon();
                body.Add(FinishNode(startMarker, new NonSpecialExpressionStatement(expr) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) }));
                break;
            }
        }
    ...
    private ImportAttribute ParseImportAttribute()
    {
        ...
        return FinishNode(startMarker, new ImportAttribute(key, value) { Range = NodeRange(startMarker), Location = NodeLocation(startMarker) });
    }
}
