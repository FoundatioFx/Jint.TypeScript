...
internal sealed partial class Tokenizer : ITokenizer
{
    ...
    public Range Range => Range.From(_startPosition, _endPosition);
    ...
    public Token Current
    {
        ...
        get => new Token(_type.Kind, _value, Range.From(_start, _end), SourceLocation.From(_startLocation, _endLocation, _sourceFile));
    }
    ...
    private void SkipBlockComment(OnCommentHandler? onComment)
    {
        ...
        onComment?.Invoke(new Comment(CommentKind.Block, Range.From(start + 2, end), Range.From(start, _position),
            SourceLocation.From(startLoc, CurrentPosition, _sourceFile)));
    }
    ...
    private void SkipLineComment(int startSkip, CommentKind kind, OnCommentHandler? onComment)
    {
        ...
        onComment?.Invoke(new Comment(kind, Range.From(start + startSkip, _position), Range.From(start, _position),
            SourceLocation.From(startLoc, CurrentPosition, _sourceFile)));
    }
    ...
    {
        // Type arguments split adjacent closers; JavaScript shifts keep the original lexer path.
        if (InType) return FinishOp(TokenType.Relational, ch == '<' ? "<" : ">");
                ...
                case '/' when !inClass:
                    ...
                    if (_options._onRegExp is { } onRegExp)
                    {
                        ...
                        parseResult = onRegExp(new RegExpParsingContext(_regExpParser,
                            Range.From(start - 1, _position),
                            SourceLocation.From(Position.From(startPosition.Line, startPosition.Column - 1), CurrentPosition, _sourceFile)));
                    }
            ...
            for (int ch; (ch = CharCodeAtPosition()) >= 0;)
            {
                switch (ch)
                {
                    ...
                    case '$' when CharCodeAtPosition(1) == '{':
                        ...
                        return FinishToken(TokenType.Template, Acornima.TemplateValue.From(templateCooked, templateRaw));
            ...
            switch (ch)
            {
                ...
                case '`':
                    ...
                    try
                    {
                        ...
                        return FinishToken(TokenType.InvalidTemplate, Acornima.TemplateValue.From(null, templateRaw));
                    }
                    finally { ReleaseStringBuilder(ref sb); }
